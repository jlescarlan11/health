...
exports.KeywordDetector = exports.SYSTEM_INDICATORS = exports.AFFIRMATIVE_KEYWORDS = exports.NEGATION_KEYWORDS = void 0;
...
var KeywordDetector = /** @class */ (function () {
    ...
    KeywordDetector.prototype.sanitizeInput = function (text) {
        ...
        for (var _i = 0, SYSTEM_INDICATORS_1 = exports.SYSTEM_INDICATORS; _i < SYSTEM_INDICATORS_1.length; _i++) {
    ...
    KeywordDetector.prototype.isNegated = function (segment, keyword) {
        ...
        var normalizedSegment = segment.toLowerCase().replace(/'/g, '-').replace(/[^a-z0-9-\s]/g, ' ');
        var words = normalizedSegment.split(/\s+/).filter(function (w) { return w.length > 0; });
        ...
        var _loop_1 = function (k) {
            ...
            if (exports.NEGATION_KEYWORDS.some(function (neg) { return currentWord === neg; })) {
                ...
                if (distance < 0 && Math.abs(distance) <= PROXIMITY_WINDOW) {
                    ...
                    var hasConjunction = intermediateWords.some(function (w) { return ['but', 'and', 'or', 'though'].includes(w); });
                    if (!hasConjunction) {
                        hasNegation = true;
                    }
                }
            }
            if (exports.AFFIRMATIVE_KEYWORDS.some(function (aff) { return currentWord === aff; })) {
    ...
    KeywordDetector.prototype.analyzeSegment = function (segment, isUserInput) {
        ...
        var maxScore = activeMatches.length > 0 ? Math.max.apply(Math, activeMatches.map(function (m) { return m.severity; })) : 0;
        return {
            text: segment,
            isUserInput: true,
    ...
    KeywordDetector.prototype.detect = function (text, isUserInput) {
        ...
        var _a = this.sanitizeInput(text), sanitized = _a.sanitized, rejected = _a.rejected;
        if (!isUserInput) {
            return {
                sanitized: sanitized,
                rejected: rejected,
        ...
        var score = segmentAnalyses.length > 0 ? Math.max.apply(Math, segmentAnalyses.map(function (s) { return s.maxScore; })) : 0;
        return {
            sanitized: sanitized,
            rejected: rejected,
