...
var ALL_EMERGENCY_KEYWORDS = __assign(
  __assign(
    __assign(__assign(__assign({}, CARDIAC_KEYWORDS), RESPIRATORY_KEYWORDS), NEURO_KEYWORDS),
    TRAUMA_KEYWORDS,
  ),
...
var EmergencyDetector = /** @class */ (function (_super) {
  ...
  EmergencyDetector.prototype.hasExclusionContext = function (contextWindow) {
    ...
    return CONTEXTUAL_EXCLUSIONS.some(function (pattern) {
      return lowerWindow.includes(pattern);
    });
  };
  EmergencyDetector.prototype.evaluate = function (text, options) {
    ...
    console.log(
      'Input Type: '.concat(options.isUserInput === false ? 'SYSTEM/METADATA' : 'USER INPUT'),
    );
    var profile = options.profile,
      questionId = options.questionId;
    // Use base class detection
    var detection = this.detect(text, options.isUserInput);
    var sanitized = detection.sanitized,
      rejected = detection.rejected,
      segmentAnalyses = detection.segments;
    ...
    if (options.enableExclusions !== false && options.isUserInput !== false) {
        ...
        if (segment.activeMatches.length !== originalCount) {
          segment.maxScore =
            segment.activeMatches.length > 0
              ? Math.max.apply(
                  Math,
                  segment.activeMatches.map(function (m) {
                    return m.severity;
                  }),
                )
              : 0;
      ...
      matchedKeywords = Array.from(
        new Set(
          segmentAnalyses.flatMap(function (s) {
            return s.activeMatches.map(function (m) {
              return m.keyword;
            });
          }),
        ),
      ...
      if (excludedKeywords.length > 0) {
        console.log(
          '  [Exclusions] Removed keywords due to context: '.concat(excludedKeywords.join(', ')),
        );
      }
    }
    ...
    var hasAbsoluteEmergency = matchedKeywords.some(function (k) {
      return ALL_EMERGENCY_KEYWORDS[k] === 10;
    });
    if (!hasAbsoluteEmergency && finalScore > 0) {
      ...
      var activeDanger = Object.keys(DANGER_INDICATORS).filter(function (dk) {
        ...
        var segment = segmentAnalyses.find(function (s) {
          return s.text.toLowerCase().includes(dk);
        });
        if (!segment) return false;
        // If it's a keyword match in this segment, check if it was suppressed (negated)
        var suppressed = segment.suppressedMatches.some(function (m) {
          return m.keyword.toLowerCase().includes(dk);
        });
      ...
      activeDanger.forEach(function (dk) {
        ...
        reasoningParts.push(
          'Danger indicator (+'.concat(DANGER_INDICATORS[dk], '): ').concat(dk, '.'),
        );
      });
      // 2. Viral Indicators (De-escalation)
      // Only de-escalate if the primary symptoms are "Serious" but not "Absolute"
      var hasViralSymptoms = VIRAL_INDICATORS.some(function (vk) {
        return sanitized.toLowerCase().includes(vk);
      });
      ...
      var urgency = this.parseDurationUrgency(
        (profile === null || profile === void 0 ? void 0 : profile.duration) || null,
      );
      ...
      if (finalScore !== initialScore) {
        console.log(
          '  [Scoring] Modified score from '
            .concat(initialScore, ' to ')
            .concat(finalScore, ' based on context.'),
        );
      }
    ...
    if (
      (profile === null || profile === void 0 ? void 0 : profile.symptom_category) === 'critical' &&
      finalScore < 8 &&
    ...
    if ((profile === null || profile === void 0 ? void 0 : profile.red_flags_resolved) === true) {
      ...
      var isExplicitDenial = explicitDenialPrefixes.some(function (prefix) {
        return (
          denials_1 === prefix ||
          denials_1.startsWith(''.concat(prefix, ' ')) ||
          denials_1.startsWith(''.concat(prefix, ',')) ||
          denials_1.startsWith(''.concat(prefix, '.'))
        );
      });
      // 2. Strengthened validation using isNegated for any matched keywords
      var areKeywordsNegated =
        matchedKeywords.length > 0 &&
        matchedKeywords.every(function (k) {
          return _this.isNegated(denials_1, k).negated;
        });
      ...
      if (hasValidatedDenial && isEmergency && !hasAbsoluteEmergency) {
        ...
        reasoningParts.push(
          'Authority block: Red flags denied in profile (Explicit: '
            .concat(isExplicitDenial, ', Negated: ')
            .concat(areKeywordsNegated, '). Capping at non-emergency.'),
        );
      }
    }
    // Build reasoning
    var reasoning = '';
    if (isEmergency) {
      reasoning = 'Emergency detected (score '
        .concat(finalScore, '/10). Symptoms: ')
        .concat(matchedKeywords.join(', '), '.');
      if (affectedSystems.length > 0)
        reasoning += ' Systems: '.concat(affectedSystems.join(', '), '.');
      if (combinationReason) reasoning += ' RISK: '.concat(combinationReason, '.');
    } else {
      reasoning =
        matchedKeywords.length > 0
          ? 'Non-emergency (score '
    ...
    var medical_justification =
      justificationParts.length > 0
        ? justificationParts.join('; ')
        : 'No emergency keywords detected.';
    console.log('\n--- FINAL RESULT ---');
    console.log(
      'Score: '.concat(finalScore, '/10 | Emergency: ').concat(isEmergency ? 'YES' : 'NO'),
    );
    ...
    if (isEmergency) {
      var advice = combinationReason
        ? 'CRITICAL: High risk combination detected ('.concat(
            combinationReason,
      ...
      overrideResponse = {
        ...
        user_advice: advice,
        clinical_soap: 'S: Patient reports '
          .concat(matchedKeywords.join(', '), '. O: Emergency keywords detected (')
          .concat(affectedSystems.join(', '), ')')
          .concat(
            combinationReason ? ' - Risk: '.concat(combinationReason) : '',
            '. A: Potential life-threatening condition. P: Immediate ED referral.',
          ),
        key_concerns: matchedKeywords.map(function (k) {
          return 'Urgent symptom: '.concat(k);
        }),
