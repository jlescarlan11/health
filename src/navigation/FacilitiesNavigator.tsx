...
fetch('http://127.0.0.1:7243/ingest/30defc92-940a-4196-8b8c-19e76254013a', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ location: 'FacilitiesNavigator.tsx:10', message: 'Import comparison', data: { defaultUndef: _defaultImport === undefined, namedHasExport: !!_namedImport.FacilityDirectoryScreen, namedKeys: Object.keys(_namedImport), timestamp: Date.now(), sessionId: 'debug-session', runId: 'run1', hypothesisId: 'A' } }) }).catch(() => {});
// #endregion
...
const FacilitiesNavigator = () => {
  // #region agent log
  fetch('http://127.0.0.1:7243/ingest/30defc92-940a-4196-8b8c-19e76254013a', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ location: 'FacilitiesNavigator.tsx:18', message: 'FacilitiesNavigator render', data: { screenComponentUndef: FacilityDirectoryScreen === undefined, detailsComponentDefined: !!FacilityDetailsScreen, timestamp: Date.now(), sessionId: 'debug-session', runId: 'run1', hypothesisId: 'A' } }) }).catch(() => {});
  // #endregion
  return (
    <Stack.Navigator>
      <Stack.Screen
