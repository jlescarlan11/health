...
describe('Temporal Emergency Verification', () => {
  ...
  it('renders all three emergency verification buttons with correct labels', async () => {
    ...
    await waitFor(() => expect(queryByText('Preparing your assessment...')).toBeNull(), { timeout: 10000 });
  ...
  it('accessibility attributes are correctly applied to all three buttons', async () => {
    ...
    await waitFor(() => {
      ...
      expect(yesBtn.props.accessibilityHint).toBe('Escalates to immediate emergency recommendation');
      ...
      expect(recentBtn.props.accessibilityHint).toBe('Continues assessment but flags the symptom as high priority');
  ...
  it('selection of "Yes" triggers 911 escalation logic (Regression)', async () => {
    ...
    await waitFor(() => expect(getByText('Yes, happening right now')).toBeTruthy(), { timeout: 5000 });
  ...
  it('selection of "No" resumes standard flow (Regression)', async () => {
    ...
    await waitFor(() => expect(getByText('No, not experiencing this')).toBeTruthy(), { timeout: 5000 });
  ...
  it('selection of "Recent" bypasses 911 but continues assessment', async () => {
    ...
    await waitFor(() => expect(getByText('Happened recently but has stopped')).toBeTruthy(), { timeout: 5000 });
...
describe('GeminiClient Temporal Logic', () => {
  ...
  it('upgrades care level to hospital when is_recent_resolved is true', async () => {
    ...
    const result = await geminiClient.assessSymptoms('unique symptom A', [], undefined, profile as any);
  ...
  it('downgrades emergency to hospital when is_recent_resolved is true', async () => {
    ...
    const result = await geminiClient.assessSymptoms('unique symptom B', [], undefined, profile as any);
